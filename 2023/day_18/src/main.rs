...
fn two(puzzle_input: &str) -> usize {
    ...
    for line in puzzle_input.lines() {
        ...
        split.next().unwrap();
        split.next().unwrap();
        let hexcode_str = split.next().unwrap();
        let magnitude = isize::from_str_radix(&hexcode_str[2..hexcode_str.len() - 2], 16).unwrap();
        let direction = usize::from_str_radix(
            &hexcode_str[hexcode_str.len() - 2..hexcode_str.len() - 1],
            16,
        )
        .unwrap();
        if direction == 3 {
